        ...
        for(unsigned i = 0; i < 6; i++) {
            ...
            while( current < P.size() ) {
                ...
                if(P[current].x() > rightOfCurrentStrip) {
                    int jump = (P[current].x() - rightOfCurrentStrip) / sqrt3;
                ...
                for(unsigned j = indexOfTheFirstPointInTheCurrentStrip; j < current; j++) {
                    ...
                    segments.push_back( Segment( Point(xOfRestrictionline,P[j].y()+y),Point(xOfRestrictionline,P[j].y()-y)));
                }
                ...
                for( unsigned k = 1; k < segments.size(); k++)
                    if( segments[k].source().y() < lowestY ) {
                        tempC.push_back(Point(xOfRestrictionline,lowestY));
