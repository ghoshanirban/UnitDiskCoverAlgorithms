...
class FASTCOVER {
    ...
    struct BoundingBox {
        ...
        BoundingBox(const Point p) : minX(p.x()), maxX(p.x()), minY(p.y()), maxY(p.y()) { }
    ...
    inline bool trytoMergeDisk(HashMap &H, HashMap::iterator &iterToSourceDisk, int vPrime, int hPrime, std::list<Point> &diskCenters) {
        HashMap::iterator iterToTargetDisk = H.find(std::make_pair(vPrime,hPrime));
    ...
    double execute() {
        ...
        for(auto aPair: H)
            if(aPair.second.second)
                diskCenters.push_back(Point(aPair.first.first*sqrt2+additiveFactor,aPair.first.second*sqrt2+additiveFactor));
